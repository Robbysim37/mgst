...
const StaffCard:React.FC<Props> = (props) => {
  ...
  const deleteStaffClickHandler = (e:React.MouseEvent<HTMLLIElement>) => {
    ...
    axios.delete('http://localhost:8000/deleteStaff',{data:{data:{data:props.children,token,username}}})
    .then(response => {
  ...
  const resetStaffPasswordClickHandler = (e:React.MouseEvent<HTMLLIElement>) => {
    menuClose()
    props.setIsLoading(true)
    axios.put(`http://localhost:8000/resetStaffPassword`,{data:props.children,token,username})
    .then(response => {
